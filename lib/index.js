...
function resolveList(buildFn, options, onfile, done) {
    ...
    function createHandler(fieldname) {
        ...
        return function(file, cb) {
            ...
            if (options.filterRx && !options.filterRx.test(name))
                return cb();
               
            if (isLocal(file.branch, options)) {

                if (options.localFilterRx && !options.localFilterRx.test(name))
                    return cb();
                ...
                if (options.remoteFilterRx && !options.remoteFilterRx.test(name))
                    return cb();
